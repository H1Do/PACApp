...
class DataProcessor(ABC):
    ...
    def run(self):
        ...
        cleaned_result = self.sort_data_by_col(self._dataset, 'player_id', True)

        # фильтруем строки по каждому возможному значению колонки "target" и вычисляем среднее4
        #name_player = name_player.strip()

        #mean_result = self.get_mean_value_by_filter(cleaned_result, f"name == '{name_player}'") # ищем игрока
        #if mean_result.empty == True:
        #    name_player += ' '
        #    mean_result = self.get_mean_value_by_filter(cleaned_result, f"team == '{name_player}'") # ищем команду
        # полученный результат (DataFrame) на каждой итерации добавляем в результирующий DataFrame в self.result
        
        html_table = cleaned_result.to_html()
